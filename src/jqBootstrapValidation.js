    ...
    var createdElements = [],
        defaults = {
            options: {
                ...
                scrollToError: false, // automatically scroll to first error on page after invalid submit attempt
            ...
            methods: {
                init: function (options) {
                    ...
                    $(uniqueForms).bind("submit.validationSubmit", function (e) {
                        var $form = $(this),
                            warningsFound = 0,
                            $allInputs = $form.find("input,textarea,select").not("[type=submit],[type=image]").filter(settings.options.filter),
                            $allControlGroups = $form.find(".form-group"),

                        // Only trigger validation on the ones that actually _have_ validation
                            $inputsWithValidators = $allInputs.filter(function () {
...
//                            $form.addClass("has-error");
                            ...
                            if (settings.options.scrollToError) {
                    ...
                    return this.each(function () {
                        ...
                        $this.bind("validation.validation",
                                    ...
                                    if (
                                        value || // has a truthy value
                                            ...
                                            ( // am including empty values
                                                (
                                                    params && params.includeEmpty) || !!settings.validatorTypes[validatorType].includeEmpty) || ( // validator is blocking submit
                                            !!
                        ...
                        $this.bind(
                            (
                                settings.options.bindEvents.length > 0 ? settings.options.bindEvents : ["keyup", "focus", "blur", "click", "keydown", "keypress", "change"]).concat(["revalidate"]).join(".validation ") + ".validation",
    ...
    var scrollToFirstError = function (form) {
        $('html, body').animate({
            scrollTop: form.find('.form-group.has-error').offset().top
        }, 'fast');
    };
